...
const customTooltips = function (context) {
  ...
  if (!this._chart.canvas.closest('.dm-chart-container').contains(tooltipEl)) {
    tooltipEl = document.createElement('div');
    tooltipEl.className = 'chartjs-tooltip';
    tooltipEl.innerHTML = '<table></table>';

    document.querySelectorAll('.dm-chart-container').forEach((el) => {
      if (el.contains(document.querySelector('.chartjs-tooltip'))) {
        document.querySelector('.chartjs-tooltip').remove();
      }
    });

    this._chart.canvas.closest('.dm-chart-container').appendChild(tooltipEl);
  }
...
const apexchartCustomTooltips = function ({ series, dataPointIndex, w }) {
  return `<div class="arrow_box dm-apex-tooltip" style="padding: 8px"> 
    <h4>${
    ...
    ${series
      .map(
        (el, index) =>
          `<p style="margin: 0px;"><span style="background-color: ${w.config.series[index].color}; width: 10px; height: 10px; margin-right: 7px; display: inline-block; border-radius: 50%;"></span>${w.config.series[index].name}: ${el[dataPointIndex]}K</p>`,
      )
      .join('')}    
  </div>`;
};

export { textRefactor, chartLinearGradient, customTooltips, apexchartCustomTooltips };
